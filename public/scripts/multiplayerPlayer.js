...
export function renderJoin(root, ws, playerId) {
  ...
  const submitJoin = () => {
    ...
    const nickname = sanitizeNickname(document.getElementById('joinNick')?.value) || 'Player';
  ...
  document.getElementById('joinBtn')?.addEventListener('click', submitJoin);

  choiceButtons().forEach((btn) => {
    btn.addEventListener('click', () => {
      ws.send('submit_answer', { roomCode, questionInstanceId, choice: btn.dataset.choice, clientSentTs: Date.now() });
    });
  });
  ...
  ws.on('connection_state', (state) => {
    ...
    if (state.connected && prefilledRoom && !roomCode) {
      submitJoin();
    }
  });
