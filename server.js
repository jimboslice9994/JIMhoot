...
const { loadWsModule } = require('./lib/wsRuntime');
...
const MAX_PLAYERS_PER_ROOM = Math.max(2, Math.min(100, Number(process.env.MAX_PLAYERS_PER_ROOM || 20)));
const MAX_WS_CLIENTS = Math.max(10, Number(process.env.MAX_WS_CLIENTS || 240));
...
const WebSocketServer = wsModule?.WebSocketServer || null;
...
function validateWsPayload(event, payload) {
  ...
  if (['join_room', 'rejoin_room'].includes(event)) {
...
function canProgressRoomState(room, nextState) {
  return canTransition(room.state, nextState);
}
...
function boundPlayerIdForRoom(ws, roomCodeRaw) {
  const conn = connBySocket.get(ws);
  if (!conn) return null;
  const roomCode = sanitizeRoomCode(roomCodeRaw);
  if (!roomCode || conn.roomCode !== roomCode) return null;
...
function handleJoin(ws, payload) {
  ...
  if (player) {
    return emitError(ws, 'UNAUTHORIZED', 'Player ID already exists; use rejoin flow');
  }
...
function handleStartGame(ws, payload) {
  ...
  const playerId = boundPlayerIdForRoom(ws, payload.roomCode);
  if (!playerId) return emitError(ws, 'UNAUTHORIZED', 'Socket is not bound to that room');
  if (playerId !== room.hostPlayerId) return emitError(ws, 'NOT_HOST', 'Only host can start game');
...
function handleNextQuestion(ws, payload) {
  ...
  const playerId = boundPlayerIdForRoom(ws, payload.roomCode);
  if (!playerId) return emitError(ws, 'UNAUTHORIZED', 'Socket is not bound to that room');
  if (playerId !== room.hostPlayerId) return emitError(ws, 'NOT_HOST', 'Only host can advance');
...
function handleDisconnect(ws) {
  ...
  const roomHasConnectedPlayers = [...room.players.values()].some((p) => p.ws && p.ws.readyState === p.ws.OPEN);
  if (!roomHasConnectedPlayers) {
    clearRoomTimers(room);
    rooms.delete(room.code);
    log('room_reaped_empty', { roomCode: room.code });
    return;
  }
...
const server = http.createServer((req, res) => {
  ...
  (async () => {
    ...
    fs.stat(filePath, (err, stats) => {
      if (err || !stats.isFile()) {
        if (pathname.startsWith('/data/') || pathname.startsWith('/scripts/') || pathname.startsWith('/styles/') || pathname.startsWith('/vendor/')) {
          res.writeHead(404);
          res.end('Not found');
          return;
        }
...
if (!WebSocketServer) {
  log('ws_disabled', { reason: 'ws package is not installed. Realtime mode unavailable until dependencies are installed.' });
} else {
  wss.on('connection', (ws) => {
    if (wss.clients.size > MAX_WS_CLIENTS) {
      safeSend(ws, 'error', { code: 'SERVER_BUSY', message: 'Server is at capacity. Please retry shortly.', details: {} });
