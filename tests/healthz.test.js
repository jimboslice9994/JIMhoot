...
test('serves vendor assets under /vendor', async () => {
  const res = await fetch(`${BASE}/vendor/qrcode.min.js`);
  assert.equal(res.status, 200);
  const body = await res.text();
  assert.ok(body.includes('QRCode'));
});

test('returns 404 for missing vendor asset', async () => {
  const res = await fetch(`${BASE}/vendor/not-found.js`);
