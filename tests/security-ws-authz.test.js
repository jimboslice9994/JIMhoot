...
test('join_room cannot hijack an existing playerId in room', async () => {
  const hostWs = await connectWs();
  sendEvent(hostWs, 'join_room', {
    role: 'host',
    playerId: 'host_hijack_guard',
    nickname: 'Host',
    deckId: 'bio_mastery',
    gameMode: 'classic',
    timerSec: 10,
  ...
  const legitPlayerWs = await connectWs();
  sendEvent(legitPlayerWs, 'join_room', {
    role: 'player',
    roomCode,
    playerId: 'victim_player',
    nickname: 'Victim',
  });
  await waitForEvent(legitPlayerWs, 'lobby_state');

  const attackerWs = await connectWs();
  sendEvent(attackerWs, 'join_room', {
    role: 'player',
    roomCode,
    playerId: 'victim_player',
    nickname: 'Attacker',
  ...
  const err = await waitForEvent(attackerWs, 'error');
